...
- **FR-1.12** يجب أن يتمكن من عرض بيانات أرشيفية لكل حلقة: اسم الحلقة، المعلم، الطلاب/ات المنظمين.

#### 3.1.5 دعم الإستيراد للإدخال الجماعي من الملفات

- **FR-1.13** يجب أن يدعم التطبيق رفع ملفات Excel لإدخال بيانات جماعية للطلاب أو المعلمين، شريطة توافق التنسيق مع القالب المعتمد.
- **FR-1.14** يجب أن يُظهر النظام تقريرًا بعد كل عملية رفع يبيّن عدد الصفوف الصحيحة، الأخطاء، والمدخلات المكررة.


#### 3.1.4 تقارير وإحصاءات الإدارة

- **FR-1.15** يجب أن يستطيع المشرف توليد تقارير يومية أو شهرية حول الأداء الجماعي للحلقات والمعلمين.
- **FR-1.16** يجب أن يتمكن من الاطلاع على مؤشرات إجمالية مثل:
...
- **FR-1.17** يجب أن يستطيع تصدير أي تقرير بصيغتي PDF وExcel.
...
- **FR-2.2** يجب أن يتلقى المعلم رسالة اخطار على التطبي عند تعديل بياناته  واخطار في حال جلسة تسجيل دخول جديدة، و رسالة Email عند إعادة تعيين كلمة مروره.
...
- **FR-2.10** يجب أن يستطيع المعلم تسجيل تقرير شهري لكل طالب/ة في حلقته، يشمل:
...
- **FR-2.12** يجب أن يستطيع المعلم مراجعة تقارير الأشهر السابقة لطالب/ة معين من خلال واجهة مبسطة.

---

### 3.3 وحدات الداشبورد والتسجيل (Registration & Web Dashboard Modules)

يتكوّن هذا الجزء من النظام منصة التسجيل العامة (موجهة للطلاب والمعلمين لتقديم طلبات الانضمام).
...
- **FR-3.3** يجب أن تُرسل إشعارات إلى تطبيق المشرف عند ورود طلب جديد.
- **FR-3.4** يجب أن يمنع النظام التقديم المكرر لنفس الرقم أو البريد أو الاسم.
- **FR-3.5** يجب أن يتلقى مقدم الطلب رسالة Email تفيد بوصول طلبه وانتظار المراجعة الإدارية وكذلك رسالة الرد بالقبول او الرفض.


تمثل وحدة التسجيل هذه البوابة الأولى للنظام من جهة المستخدمين الخارجيين.
...
- **الهدف**: توثيق أداء الطالب/ة اليومي في الحفظ والمراجعة والغياب
...
ملاحظتك دقيقة جدًا، وما تم تضمينه في الملف يغطي فقط **السيناريوهات الأساسية**، ولا يُعد كافيًا لتغطية كافة حالات الاستخدام الضرورية في نظام مؤسسي متكامل مثل "أكاديمية تاج الوقار". لذا سأقوم الآن **بتوسيع حالات الاستخدام (Use Cases)** بما يغطي جميع الوحدات التي تم تحليلها في وثيقة المتطلبات، وسأكمل بنفس المنهجية الأكاديمية الدقيقة:



###  Use Case 7: مراجعة طلبات التسجيل (المشرف)

* **الممثل الأساسي**: المشرف
* **الهدف**: فحص طلبات التسجيل الجديدة للطلاب والمعلمين عبر المنصة وقبولها أو رفضها
...
###  Use Case 8: تعديل بيانات الطالب أو المعلم
...
* **الخطوات الأساسية**:
  ...
  2. يفتح ملف المستخدم.
  3. يعدّل الحقول المسموح بها (مثل: رقم الجوال، المؤهل).
  4. يحفظ التغييرات.
* **نقطة النهاية**: تُحدّث البيانات في قاعدة البيانات.



###  Use Case 9: تصدير التقارير والإحصاءات

* **الممثل الأساسي**: المشرف
* **الهدف**: استخراج ملفات بتنسيقات Excel أو PDF للاستخدام الإداري أو التوثيق
...
###  Use Case 10: تنبيه المعلم بتجاوز عدد غيابات الطالب
...
### Use Case 11: أرشفة الحلقات غير النشطة
...
### Use Case 12: استعراض سجل الأداء لطالب
...
###  Use Case 13: رفع بيانات الطلاب/المعلمين من ملف Excel

* **الممثل الأساسي**: المشرف
* **الهدف**: إدخال بيانات جماعية دون الحاجة للإدخال اليدوي
* **الشروط المسبقة**: ملف Excel مطابق للقالب المعتمد
* **الخطوات الأساسية**:
...
###  Use Case 14: إرسال ملاحظات من المعلم للمشرف
...
- **UI-11** شاشة توزيع الطلاب: عرض قائم على جدول بواجهة سحب وإفلات مثلا  لتوزيع الطلاب على الحلقات.
...
- **إطار او لغة العمل:** Laravel 10.x أو PHP فقط (حسب اختيار فريق التطوير)
...
- **أطر العمل الممكنة:** Laravel ، وBootstrap لتصميم الواجهة
